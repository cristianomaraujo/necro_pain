# -*- coding: utf-8 -*-
"""Dor Endo Everdan_72hs

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1Q67ufZcLtyAySj_kHq3IOz0PB8BdxyS7
...
# @title
colunas = ['Irrigation solution', 'Foraminal enlargement', 'Sex','Irrigant activation',
       'Age range','Postoperative pain 72 hours (dichotomous)']

df = df.loc[:, colunas]

coluna_grupo = df.pop('Postoperative pain 72 hours (dichotomous)')
df.insert(0, 'Postoperative pain 72 hours (dichotomous)', coluna_grupo)
...
# Salvar o modelo final em formato PKL
joblib.dump(best_logreg_model, "logreg_24h.pkl")
...
# -------- TESTE (24h, LogReg) --------
thr_test_lr, tpr_test_lr, fpr_test_lr = best_threshold_by_youden(y_test, y_prob_test_logreg)
y_pred_thr_test_lr = (y_prob_test_logreg >= thr_test_lr).astype(int)

acc_t = accuracy_score(y_test, y_pred_thr_test_lr)
prec_t = precision_score(y_test, y_pred_thr_test_lr, zero_division=0)
rec_t  = recall_score(y_test, y_pred_thr_test_lr)
f1_t   = f1_score(y_test, y_pred_thr_test_lr)
cm_t   = confusion_matrix(y_test, y_pred_thr_test_lr)

print(f"[TESTE 24h] Melhor limiar (Youden): {thr_test_lr:.3f} | Sens(TPR): {tpr_test_lr:.3f} | 1-Espec(FPR): {fpr_test_lr:.3f}")
print(f"[TESTE 24h] Acc: {acc_t:.3f} | Precisão: {prec_t:.3f} | Recall: {rec_t:.3f} | F1: {f1_t:.3f}")
print("Matriz de confusão (teste):\n", cm_t)

# -------- CROSS-VALIDATION (24h, LogReg) --------
thr_cv_lr, tpr_cv_lr, fpr_cv_lr = best_threshold_by_youden(y_train, y_prob_cv_logreg)
y_pred_thr_cv_lr = (y_prob_cv_logreg >= thr_cv_lr).astype(int)

acc_cv = accuracy_score(y_train, y_pred_thr_cv_lr)
prec_cv = precision_score(y_train, y_pred_thr_cv_lr, zero_division=0)
rec_cv  = recall_score(y_train, y_pred_thr_cv_lr)
f1_cv   = f1_score(y_train, y_pred_thr_cv_lr)
cm_cv   = confusion_matrix(y_train, y_pred_thr_cv_lr)

print(f"[CV 24h] Melhor limiar (Youden): {thr_cv_lr:.3f} | Sens(TPR): {tpr_cv_lr:.3f} | 1-Espec(FPR): {fpr_cv_lr:.3f}")
print(f"[CV 24h] Acc: {acc_cv:.3f} | Precisão: {prec_cv:.3f} | Recall: {rec_cv:.3f} | F1: {f1_cv:.3f}")
...
kf_svc = KFold(n_splits=3, shuffle=True, random_state=RANDOM_STATE)
...
models_test = [
    {
        ...
        "fpr": fpr_svc, "tpr": tpr_svc,
        "auc": roc_auc_svc, "ci": auc_ci_test_sv,
        "color": "green"
    },
    {
        "name": "Multilayer Perceptron",
        "fpr": fpr_mlp, "tpr": tpr_mlp,
        "auc": roc_auc_mlp, "ci": auc_ci_test_mlp,
        "color": "purple"
    },
    {
        "name": "Random Forest",
        "fpr": fpr_rf, "tpr": tpr_rf,
        "auc": roc_auc_rf, "ci": auc_ci_test_rf,
        "color": "deeppink"
    },
    {
        "name": "Gradient Boosting",
        "fpr": fpr_gb, "tpr": tpr_gb,
        "auc": roc_auc_gb, "ci": auc_ci_test_gb,
        "color": "orange"
    },
    {
        "name": "Logistic Regression",
        "fpr": fpr_logreg, "tpr": tpr_logreg,
        "auc": roc_auc_logreg, "ci": auc_ci_test_lr,
        "color": "red"
    },
    {
        "name": "Decision Tree",
        "fpr": fpr_dt, "tpr": tpr_dt,
        "auc": roc_auc_dt, "ci": auc_ci_test_dt,
        "color": "darkred"
    },
    {
        "name": "K Nearest Neighbors",
        "fpr": fpr_knn, "tpr": tpr_knn,
        "auc": roc_auc_knn, "ci": auc_ci_test_knn,
        "color": "blue"
    },
    {
        "name": "Adaboost Classifier",
        "fpr": fpr_ada, "tpr": tpr_ada,
        "auc": roc_auc_ada, "ci": auc_ci_test_ada,
        "color": "brown"
    }
]

# ordenar por AUC (decrescente)
models_test_sorted = sorted(models_test, key=lambda x: x["auc"], reverse=True)

# plot
for m in models_test_sorted:
    axs[0].plot(
        m["fpr"], m["tpr"],
        lw=2,
        color=m["color"],
        label=f'{m["name"]} (AUC = {m["auc"]:.2f}, CI 95%: [{m["ci"][0]:.2f}, {m["ci"][1]:.2f}])'
    )
models_cv = [
    {
        "name": "Random Forest",
        "fpr": fpr_cv_rf, "tpr": tpr_cv_rf,
        "auc": roc_auc_cv_rf, "ci": auc_ci_cv_rf,
        "color": "deeppink"
    },
    {
        "name": "Gradient Boosting",
        "fpr": fpr_cv_gb, "tpr": tpr_cv_gb,
        "auc": roc_auc_cv_gb, "ci": auc_ci_cv_gb,
        "color": "orange"
    },
    {
        "name": "Logistic Regression",
        "fpr": fpr_cv_logreg, "tpr": tpr_cv_logreg,
        "auc": roc_auc_cv_logreg, "ci": auc_ci_cv_lr,
        "color": "red"
    },
    {
        "name": "K Nearest Neighbors",
        "fpr": fpr_cv_knn, "tpr": tpr_cv_knn,
        "auc": roc_auc_cv_knn, "ci": auc_ci_cv_knn,
        "color": "blue"
    },
    {
        "name": "Multilayer Perceptron",
        "fpr": fpr_cv_mlp, "tpr": tpr_cv_mlp,
        "auc": roc_auc_cv_mlp, "ci": auc_ci_cv_mlp,
        "color": "purple"
    },
    {
        "name": "Decision Tree",
        "fpr": fpr_cv_dt, "tpr": tpr_cv_dt,
        "auc": roc_auc_cv_dt, "ci": auc_ci_cv_dt,
        "color": "gray"
    },
    {
        "name": "Adaboost Classifier",
        "fpr": fpr_cv_ada, "tpr": tpr_cv_ada,
        "auc": roc_auc_cv_ada, "ci": auc_ci_cv_ada,
        "color": "brown"
    },
    {
        "name": "Support Vector Machine",
        "fpr": fpr_cv_svc, "tpr": tpr_cv_svc,
        "auc": roc_auc_cv_svc, "ci": auc_ci_cv_sv,
        "color": "green"
    }
]

# ordenar por AUC (decrescente)
models_cv_sorted = sorted(models_cv, key=lambda x: x["auc"], reverse=True)

# plot
for m in models_cv_sorted:
    axs[1].plot(
        m["fpr"], m["tpr"],
        lw=2,
        color=m["color"],
...
X = df.drop('Postoperative pain 72 hours (dichotomous)', axis=1)
features = X.columns
